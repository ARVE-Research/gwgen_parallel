...
real(dp) :: alpha, a, c, ch, p1
real(dp) :: p2, q, t, x, lgam1pa
...
real(dp) :: a, b, c, g, ch, ch0, p1
real(dp) :: p2, q, s1, s2, s3, s4, s5, s6, t
...
real(dp), intent(in) :: p  ! the exponent parameter (0.0 < P)
...
real(dp) :: c
real(dp) :: cdf
real(dp), parameter :: exp_arg_min = -88.d0
real(dp), parameter :: overflow = 1.d+37
real(dp), parameter :: plimit = 1000.d0
real(dp) :: pn1
real(dp) :: pn2
real(dp) :: pn3
real(dp) :: pn4
...
if ( plimit < p ) then
  ...
  pn1 = 3.d0 * sqrt(p) * ((x / p)**(1.d0/3.d0) + 1.d0 / (9.d0 * p ) - 1.d0)
...
if ( x <= 1.d0 .or. x < p ) then

  arg = p * log ( x ) - x - gamma_log ( p + 1.d0 )
  c = 1.d0
  gamma_inc = 1.d0
  a = p
  ...
  do
    ...
    real(dp) function gamma_log ( x )
        ...
        ! for X < 12.d0 are similar to those in Cody and Hillstrom,
        ! but are unpublished.
        !
    ...
    subroutine gamma_pdf ( x, a, b, c, pdf )
        ...
        real(dp), intent(in) :: b    ! the "scale" parameter; 0.0 < B, and is often 1.0.
        real(dp), intent(in) :: c    ! the "shape" parameter; 0.0 < C, and is often 1.0.
        ...
        if ( x <= a ) then
            ...
            pdf = y**( c - 1.d0 ) / ( b * r8_gamma ( c ) * exp ( y ) )
    ...
    real(dp) function r8_gamma ( x )
        ...
        real(dp) fact
        real(dp), parameter :: half = 0.5D+00
        integer ( kind = 4 ) i
        integer ( kind = 4 ) n
    ...
    subroutine normal_cdf_inv ( cdf, a, b, x )
        ...
        real(dp), intent(in) :: cdf ! the value of the CDF. 0.0 <= CDF <= 1.0.
    ...
    subroutine normal_01_cdf_inv ( p, x )
        ...
        real(dp), parameter :: const1 = 0.180625D+00
        real(dp), parameter :: const2 = 1.6D+00
        ...
        real(dp) q
        real(dp) r
        real(dp), parameter :: split1 = 0.425D+00
        real(dp), parameter :: split2 = 5.d0
    ...
    real(dp) function r8poly_value_horner ( m, c, x )
        ...
        real(dp), intent(in) :: c(0:m) ! the polynomial coefficients. C(I) is the coefficient of  :math:`X^I`
