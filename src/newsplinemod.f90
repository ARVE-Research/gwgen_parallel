...
solution(1)   = 2. * monthdata(1) - swc(1) * G_00 - 0.5 * (swc(2) - swc(1)) &
          * G_11 - 0.5 * (swc(2) - swc(1)) * G_10 - swc(2) * G_01 - swc(1)
solution(1)   = solution(1) + 0.5 * G_10 * monthdata(1)

solution(len) =  2. * monthdata(len) - swc(len) * G_00 - 0.5 * (swc(len+1) - &
swc(len)) * G_11 - 0.5 * (swc(len+1) - swc(len)) * G_10 - swc(len+1) * G_01 - swc(len)
solution(len) = solution(len) - 0.5 * G_11 * monthdata(len)


do i = 2, (len-1)

  solution(i) = 2. * monthdata(i) - swc(i) * G_00 - 0.5 * (swc(i+1) - swc(i))* &
  G_11 - 0.5 * (swc(i+1) - swc(i)) * G_10 - swc(i+1) * G_01 - swc(i)
...
solution(1)   = 2. * monthdata(1) - swc(1) * G_00 - 0.5 * (swc(2) - swc(1)) * &
G_11 - 0.5 * (swc(2) - swc(1)) * G_10 - swc(2) * G_01 - swc(1) + 0.5 * G_10 * monthdata(1)

solution(len) =  2. * monthdata(len) - swc(len) * G_00 - 0.5 * (swc(len+1) - &
swc(len)) * G_11 - 0.5 * (swc(len+1) - swc(len)) * G_10 - swc(len+1) * G_01 - swc(len) - 0.5 * G_11 * monthdata(len)
...
do i = 2, (len-1)
  ...
  if (.not.slope_check(i)) then
    ...
    (d_orig(i) == -1 .AND. (all_cont(2*i) - all_cont(2*i-1)) > 0))  then
      ...
      s_sol(1) = 2 * monthdata(i) - (G_00 - 0.5 * G_10 - 0.5 * G_11 + 1) * &
      all_cont(2*i-1) + 0.5 * G_10 * all_cont(2*i-2) - (G_00 + G_01 + 0.5 * G_10 - 0.5 * G_11 + 1.) * all_cont(2*i)

      s_sol(2) = 2 * monthdata(i+1) - (G_01 + 0.5 * G_10 + 0.5 * G_11) * &
      all_cont(2*i+3) + 0.5 * G_10 * all_cont(2*i) - 0.5 * G_11 * all_cont(2*i+4)
    ...
    else if ((d_orig(i) == 1 .AND. (all_cont(2*i+1) - all_cont(2*i)) < 0.) &
      .OR. (d_orig(i) == -1 .AND. (all_cont(2*i+1) - all_cont(2*i)) > 0))  then
      ...
      s_sol(1) = 2 * monthdata(i-1) - (G_00 - 0.5 * G_10 - 0.5 * G_11 + 1.) &
      * all_cont(2*i-3) + 0.5 * G_10 * all_cont(2*i-4) - 0.5 * G_11 * all_cont(2*i)

      s_sol(2) = 2 * monthdata(i) - (G_01 + 0.5 * G_10 + 0.5 * G_11) &
      * all_cont(2*i+1) - (G_00 + G_01 + 0.5 * G_10 - 0.5 * G_11 + 1.) * all_cont(2*i) - 0.5 * G_11 * all_cont(2*i+2)
